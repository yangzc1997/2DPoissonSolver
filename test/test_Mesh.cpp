...
#include "Mesh.h"
#include "../src/Auxiliary_MeshGenerator.h"
#include <cmath>
#include <iostream>
...
TEST(MeshTest, RectangleMesh) {
    // 创建 2x2 矩形网格
    Mesh mesh = Auxiliary::MeshGenerator::generate_mesh(2.0, 1.0, 2, 2, "4");
...
TEST(MeshTest, TriangleMesh) {
    // 创建 1x1 三角形网格
    Mesh mesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, 1, 1, "3");
...
TEST(MeshTest, LargeGrid) {
    ...
    Mesh mesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, Nx, Ny, "4");
...
TEST(MeshTest, DefaultMeshType) {
    // 测试默认网格类型
    Mesh mesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, 1, 1, "invalid");
...
TEST(MeshTest, MixedMeshTypes) {
    // 测试不同网格类型
    Mesh rectMesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, 2, 2, "4");
    Mesh triMesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, 2, 2, "3");
...
TEST(MeshTest, NodeIds) {
    // 测试节点ID是否正确分配
    Mesh mesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, 2, 2, "4");
...
TEST(MeshTest, ElementNodes) {
    // 测试单元节点指针是否正确
    Mesh mesh = Auxiliary::MeshGenerator::generate_mesh(1.0, 1.0, 1, 1, "4");
