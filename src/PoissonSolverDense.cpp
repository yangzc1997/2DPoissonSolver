...
PoissonSolverDense::PoissonSolverDense(const ReadInput& read_input, const Mesh& mesh)
...
void PoissonSolverDense::assemble() 
{
    K.setZero();
    f.setZero();
...
// compute_localK 和 compute_localF 实现与稀疏求解器完全相同



void PoissonSolverDense::solveNewton() 
{
    std::cout << "\n牛顿迭代求解中... (稠密矩阵)" << std::endl;
    int iter = 0;
    ...
    while (iter <= read_input.max_iter) {
        ...
        // 使用全主元LU分解求解线性系统
        // delta_u = K.lu().solve(f);
        delta_u = K.fullPivLu().solve(f);
        u += delta_u;
